...
public class FinalInspectionBusiness {
    ...
    public boolean finalControl(Connection nc, ChannelEvent channelEvent, IVREvent ivrEvent, String callerIdNumber, String phoneAdsCode) {
        if (XCCConstants.TEST_NUMBER.equals(phoneAdsCode)) {
            ...
            //正常执行话务
            if (StringUtils.isAnyBlank(ngdEvent.getRetKey(), ngdEvent.getRetValue())) {
                return false;
            }
            //对话记录
            ivrEvent.getNgdNodeMetadataArray().add(ngdEvent.getNgdNodeMetaData());
